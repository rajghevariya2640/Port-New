             ...
             return (
               <div className='w-full sm:w-1/2 xl:w-3/12 px-3'>
                     <TeamCard teamCard={team} key={teamIndex} mainClass="pb-2.5" />
                        <div className="md:flex xl:block xxl:flex flex-wrap items-center justify-between pt-4 border-t-[1px] border-t-primary-100 group">
                    {team?.teamData?.map((team) => {
                        return (
                            <>
                          <PNButton variant="link" className="text-cstmGrey-100 font-syne font-semibold !text-lg !capitalize group-hover:text-secondary">{team?.name}</PNButton>
